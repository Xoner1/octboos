...
export async function initCommand(options) {
  ...
  if (existsSync(join(cwd, AGENT_DIR, 'config.json'))) {
    ...
    console.log(chalk.gray('     Run ' + chalk.white('npx @octboos/octboos sync') + ' to update.'));
    console.log('');
    return;
  }
  ...
  console.log(chalk.cyan('  2.') + chalk.white(' Run npx @octboos/octboos sync to update wiki'));
...
function buildWikiIndex(projectName, scanResult) {
  const lines = [
    ...
    '',
    '## Modules',
    '> Add module documentation here or run `npx @octboos/octboos sync` to auto-generate.',
    '',
    '## Stack Summary',
  ];
